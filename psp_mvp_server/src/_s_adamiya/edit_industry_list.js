...
app.post('/', function(req, res) {
  ...
  req.on('end', function () {
    ...
    if(params.custom_code && params.label && params.label !='' && params.display_code && params.display_code !='' && params.sn && params.sn !='' && params.seo_tag && params.seo_tag !='') {
      var sql = "select * from industry_info where (sn<>? and label=?) or (sn<>? and custom_code=?)";
      ...
      connection.query(sql, sqlpara, function (err, rows, fields) {
        if (err) {
          ...
          if (rows.length === 0) {
            var addSql = "update industry_info set custom_code=?,label=?,seo_tag=?,display_code=? where sn=?";
