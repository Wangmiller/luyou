    ...
    connection.query(sql, sqlpara, function (err, rows, fields) {
      if (err) {
        ...
        if (rows.length === 0) {
          var lll = new lib();
          var sn = lll.GetUUID();
          var addSql = "insert into user_info (sn,label,login_name,type_sn,position,mobile,email,tel) values (?,?,?,'master',?,?,?,?)";
          var addSqlParams = [sn, params.label, params.login_name, params.position, params.mobile, params.email, params.tel];
          // 设置响应头部信息及编码
