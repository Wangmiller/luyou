...
app.post("/",function(req,res,next){
  var form = new multiparty.Form({uploadDir: './upload/industry/'});
  form.parse(req, function(err, fields, files) {
    ...
    if(err){
      ...
      var dstPath = './upload/industry/' + inputFile.originalFilename;
      //重命名为真实文件名
      fs.rename(uploadedPath, dstPath, function(err) {
        if(err){
          ...
          fs.readFile(dstPath, function (err, data) {
            ...
            var rows = new Array();
            var cols = new Array();
            var addSql = 'insert into industry_info (sn,label) values (replace(uuid(),\'-\',\'_\'),?)';
            rows = data.split("\r\n");
            // 首行不导入
            ...
            res.end(util.inspect({成功导入行业: okcount.toString(), 导入失败: failcount.toString()}));
          });
        }
      });
    }
