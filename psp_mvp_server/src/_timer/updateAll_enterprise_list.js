...
const {timer} = require('./timer_update_enterprise');
        ...
        if (err) {
            ...
            if (rows.length > 0) {
                for (var i = 0; i < rows.length; i++) {
                    ...
                    getEnterprise(rows[i].label, rows[i].id, index, rows[i].sn);
                }
            } else {
                res.send({'ststus': '暂无企业可更新'});
            }
        }
    });

    function getEnterprise(lable, id, index, sn){
        ...
        if(arr.data[0].company_name == lable){
            getEnterpriseInfo(arr.data[0]._id, id, index, sn);
        }else {
            if(index == 'last'){
                res.send({'status': '更新企业信息成功'});
            }
        }
        // getEnterpriseInfo(arr.data[0]._id,id);
    }
    function getEnterpriseInfo(_id, id, index, sn){
        /*router.post('/_timer/enterprise.json',function (req, res, next) {
            // 打印post请求的数据内容
            console.log(req.body);
            console.log(req.body.keyword);
        ...
        var str = "{\n" +
            ...
            "}";
        var arr = JSON.parse(str);
        updateEnterpriseInfo(arr, id, index, sn);
    }
    function updateEnterpriseInfo(arr, id, index, sn){
                ...
                if (rows.length > 0) {
                    ...
                    connection.query(addSql, addSqlParams, function (err, result) {
                        if(err){
                            ...
                            console.log('手动更新企业"' + arr.data[0].company_logo_name + '"失败');
                            res.send({'status': '更新企业信息失败'});
                        }
                        else {
                            console.log('手动更新企业"' + arr.data[0].company_logo_name + '"成功');
                            if (index == 'last') {
                                res.send({'status': '更新企业信息成功'});
                            }
                        }
                ...
                if (rows.length > 0) {
                    ...
                    connection.query(inInSql, inInSqlParams, function (err, result) {
                        if (err) {
                            ...
                            var inSqlEnIn = "insert into cr_enterprise_industry (sn,industry_sn,enterprise_sn) values (?,?,?)";
                            var inEnInSqlParams = [cr_en_in_sn, industry_sn, sn];
