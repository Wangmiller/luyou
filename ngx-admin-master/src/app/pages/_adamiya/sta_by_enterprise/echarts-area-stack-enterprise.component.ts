...
@Component({
  selector: 'ngx-echarts-area-stack-enterprise',
  template: `
    <div echarts [options]="options" class="echart"></div>
  `,
})
export class EchartsAreaStackEnterpriseComponent implements DoCheck, OnDestroy {
  options: any = {};
  ...
  ngDoCheck() {
    const ls0 = localStorage.getItem('sta_by_enterprise_grow_industry_dt');
    if (ls0 && ls0 === '1') {
      localStorage.setItem('sta_by_enterprise_grow_industry_dt', '');
      const ls1 = localStorage.getItem('sta_by_enterprise_xsld_year_month');
      // window.alert(ls1);
      const ls2 = localStorage.getItem('sta_by_enterprise_xsld_industry');
      // window.alert(ls2);
      const ls3 = localStorage.getItem('sta_by_enterprise_xssd_grow_industry');
      // window.alert(ls3);
