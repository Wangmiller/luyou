...
@Component({
  selector: 'ngx-echarts-pie-scene-product',
  template: `
    <div echarts [options]="options" class="echart"></div>
  `,
})
export class EchartsPieSceneProductComponent implements DoCheck, OnDestroy {
  options: any = {};
  ...
  ngDoCheck() {
    const ls0 = localStorage.getItem('sta_by_product_scene_dt');
    if (ls0 && ls0 === '1') {
      localStorage.setItem('sta_by_product_scene_dt', '');
      const ls1 = localStorage.getItem('sta_by_product_xssd_scene');
      // window.alert(ls1);
      const ls2 = localStorage.getItem('sta_by_product_xsld_scene');
      // window.alert(ls2 + 's');
