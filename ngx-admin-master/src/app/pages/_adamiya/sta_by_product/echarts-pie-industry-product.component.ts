...
@Component({
  selector: 'ngx-echarts-pie-industry-product',
  template: `
    <div echarts [options]="options" class="echart"></div>
  `,
})
export class EchartsPieIndustryProductComponent implements DoCheck, OnDestroy {
  options: any = {};
  ...
  ngDoCheck() {
    const ls0 = localStorage.getItem('sta_by_product_industry_dt');
    if (ls0 && ls0 === '1') {
      localStorage.setItem('sta_by_product_industry_dt', '');
      const ls1 = localStorage.getItem('sta_by_product_xssd_industry');
      // window.alert(ls1);
